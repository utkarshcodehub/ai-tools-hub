...
export default function ComparePage() {
  ...
  const paidTools = useMemo(() =>
    allTools?.filter(t => !t.pricing.has_free_tier) || [], [allTools])

  const filtered = useMemo(() => {
    if (!allTools) return []
    if (!search) return paidTools
    const q = search.toLowerCase()
    return paidTools.filter(t =>
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 flex flex-col gap-8">
      ...
      <div>
        <h1 className="text-2xl font-semibold text-gray-900 mb-1">Paid vs free</h1>
        <p className="text-sm text-gray-500">
          Find free alternatives for paid tools. Select up to 3 tools to compare.
        </p>
      </div>
      ...
      <div>
        <p className="text-xs text-gray-400 mb-3">
          {filtered.length} paid tool{filtered.length !== 1 ? 's' : ''} — click to select
        </p>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {filtered.map(tool => {
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-red-500">{tool.pricing.paid_starts_at}</p>
                </div>
              </button>
            )
          })}
      ...
      {selected.length > 0 && (
        <div className="flex flex-col gap-5">
          <h2 className="text-sm font-semibold text-gray-800">Free alternatives</h2>
          {selected.map(tool => {
            const alts = getAlternatives(tool)
            return (
              <div key={tool.id} className="bg-white border border-gray-200 rounded-xl p-5">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">Starting at {tool.pricing.paid_starts_at}</p>
                  </div>
                </div>
                ...
                {alts.length > 0 ? (
                  ...
                  <p className="text-sm text-gray-400">No mapped free alternatives yet.</p>
                )}
              </div>
            )
          })}
