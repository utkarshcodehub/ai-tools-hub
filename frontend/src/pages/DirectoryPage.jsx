...
export default function DirectoryPage() {
  const { data: allTools, loading, error, retry } = useFetch(fetchTools, [])
  ...
  return (
    <>
      <SEOHead
        ...
        description="Browse and filter all AI tools by category, free tier, and API availability."
        path="/directory"
      />
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 flex flex-col gap-6">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Browse and filter all {allTools?.length || 0} tools
          </p>
        </div>
  ...
  <ErrorMessage message="Could not load the directory. Is the backend running?" onRetry={retry} />
) : displayed.length > 0 ? (
